...
def update_params(n_clicks, params_table):
    if n_clicks == 0:
        raise PreventUpdate()
    try:
        params_with_units = get_simulation_parameters(PARAMS_PATH, with_units=True)
        ...
        for key in params_with_units.keys():
            params_with_units[key] = (new_params_no_units[key], params_with_units[key][1])
        with open(PARAMS_PATH, 'w', newline='\n') as csvfile:
            writer = csv.writer(csvfile, delimiter=',')
            for k, v in params_with_units.items():
