...
def run_simulation(n_clicks, num_batteries, solar_panel_power_mw, simulated_year, chosen_strategy, place_to_research):
    ...
    if n_clicks == 0:
        return {}, False
    try:
        solar_panel_power_mw = float(solar_panel_power_mw)
        num_batteries = float(num_batteries)
        simulated_year = int(simulated_year)
    except:
        return {}, True
    if (not place_to_research) or (
            not chosen_strategy) or solar_panel_power_mw < 0 or num_batteries < 0 or simulated_year < 0:
        return {}, True
    ...
    for_download[normalised_production.SolarProduction] = normalised_production.df[
                                                              normalised_production.SolarProduction] * solar_panel_power_mw * 1000  # productino in kw
